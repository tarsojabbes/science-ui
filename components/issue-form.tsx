'use client'
import { cn } from "@/lib/utils"
import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import { Label } from "@/components/ui/label"
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from "@/components/ui/dropdown-menu"
import { useState, useEffect } from "react"
import axios from "axios"
import { useRouter, useSearchParams } from "next/navigation"
import { ChevronDownIcon, XIcon } from "lucide-react"

type Journal = {
    id: number
    name: string
    issn: string
}

type Paper = {
    id: number
    name: string
    journalId: number
    status: string
    publishedDate: string
    submissionDate: string
}

export function IssueForm({
    className,
    ...props
}: React.ComponentProps<"form">) {
    const [number, setNumber] = useState<number>(1)
    const [volume, setVolume] = useState<number>(1)
    const [selectedJournal, setSelectedJournal] = useState<Journal | null>(null)
    const [selectedPapers, setSelectedPapers] = useState<Paper[]>([])
    const [availablePapers, setAvailablePapers] = useState<Paper[]>([])
    const [journals, setJournals] = useState<Journal[]>([])
    const [loading, setLoading] = useState(false)
    const [error, setError] = useState<string | null>(null)
    const router = useRouter()
    const searchParams = useSearchParams()
    const journalIdFromUrl = searchParams.get('journalId')

    useEffect(() => {
        const fetchData = async () => {
            try {
                const server = process.env.NEXT_PUBLIC_SERVER_DOMAIN
                const token = window?.localStorage.getItem("@science.token")

                const journalsResponse = await axios.get(`${server}/journals?limit=100`, {
                    headers: { "Authorization": "Bearer " + token }
                })
                setJournals(journalsResponse.data.data || [])

                if (journalIdFromUrl) {
                    const journal = journalsResponse.data.data?.find((j: Journal) => j.id === parseInt(journalIdFromUrl))
                    if (journal) {
                        setSelectedJournal(journal)
                    }
                }

            } catch (err) {
                console.error('Erro ao buscar dados:', err)
                setError('Erro ao carregar dados')
            }
        }

        fetchData()
    }, [journalIdFromUrl])

    useEffect(() => {
        const fetchPapers = async () => {
            if (!selectedJournal) {
                setAvailablePapers([])
                return
            }

            try {
                const server = process.env.NEXT_PUBLIC_SERVER_DOMAIN
                const token = window?.localStorage.getItem("@science.token")

                const papersResponse = await axios.get(`${server}/papers/journal/${selectedJournal.id}`, {
                    headers: { "Authorization": "Bearer " + token }
                })
                const papersData = papersResponse.data
                console.log('Papers response:', papersData)
                let papers = []
                if (Array.isArray(papersData)) {
                    papers = papersData
                } else if (papersData && Array.isArray(papersData.data)) {
                    papers = papersData.data
                } else {
                    console.warn('Unexpected papers response structure:', papersData)
                    papers = []
                }
                
                const approvedPapers = papers.filter((paper: Paper) => paper.status === 'approved')
                setAvailablePapers(approvedPapers)
            } catch (err) {
                console.error('Erro ao buscar papers:', err)
                setAvailablePapers([])
            }
        }

        fetchPapers()
    }, [selectedJournal])

    const handleAddPaper = (paper: Paper) => {
        if (!selectedPapers.find(p => p.id === paper.id)) {
            setSelectedPapers(prev => [...prev, paper])
        }
    }

    const handleRemovePaper = (paperId: number) => {
        setSelectedPapers(prev => prev.filter(p => p.id !== paperId))
    }

    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault()
        
        if (!selectedJournal) {
            setError('Please select a journal')
            return
        }
        
        if (selectedPapers.length === 0) {
            setError('Please select at least one paper')
            return
        }

        setLoading(true)
        setError(null)

        try {
            const server = process.env.NEXT_PUBLIC_SERVER_DOMAIN
            const token = window?.localStorage.getItem("@science.token")

            const issueData = {
                number,
                volume,
                journalId: selectedJournal.id,
                paperIds: selectedPapers.map(p => p.id)
            }

            await axios.post(`${server}/issues`, issueData, {
                headers: { 
                    "Authorization": "Bearer " + token,
                    "Content-Type": "application/json"
                }
            })

            router.push("/home")
        } catch (err) {
            console.error('Erro ao criar issue:', err)
            setError('Erro ao criar a issue')
        } finally {
            setLoading(false)
        }
    }

    const availableToAdd = Array.isArray(availablePapers) ? availablePapers.filter(
        paper => !selectedPapers.find(selected => selected.id === paper.id)
    ) : []

    return (
        <form 
            onSubmit={handleSubmit}
            className={cn("flex flex-col gap-6", className)} 
            {...props}
        >
            <div className="flex flex-col items-center gap-2 text-center">
                <h1 className="text-2xl font-bold">Create a new issue</h1>
            </div>
            
            <div className="grid gap-6">
                {/* Journal Selection */}
                <div className="grid gap-3">
                    <Label>Journal *</Label>
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="outline" className="w-full justify-between">
                                {selectedJournal ? (
                                    <div className="flex flex-col items-start">
                                        <span className="font-medium">{selectedJournal.name}</span>
                                        <span className="text-xs text-gray-500">ISSN: {selectedJournal.issn}</span>
                                    </div>
                                ) : (
                                    <span className="text-gray-500">Select a journal</span>
                                )}
                                <ChevronDownIcon className="h-4 w-4" />
                            </Button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent className="w-full min-w-[400px]">
                            {journals.map((journal) => (
                                <DropdownMenuItem 
                                    key={journal.id}
                                    onClick={() => setSelectedJournal(journal)}
                                    className="flex flex-col items-start cursor-pointer"
                                >
                                    <span className="font-medium">{journal.name}</span>
                                    <span className="text-xs text-gray-500">ISSN: {journal.issn}</span>
                                </DropdownMenuItem>
                            ))}
                        </DropdownMenuContent>
                    </DropdownMenu>
                </div>

                {/* Number and Volume */}
                <div className="grid grid-cols-2 gap-4">
                    <div className="grid gap-3">
                        <Label htmlFor="number">Issue Number *</Label>
                        <Input 
                            id="number" 
                            type="number" 
                            required 
                            min="1"
                            value={number}
                            onChange={(e) => setNumber(parseInt(e.target.value) || 1)}
                            placeholder="1"
                        />
                    </div>
                    <div className="grid gap-3">
                        <Label htmlFor="volume">Volume *</Label>
                        <Input 
                            id="volume" 
                            type="number" 
                            required 
                            min="1"
                            value={volume}
                            onChange={(e) => setVolume(parseInt(e.target.value) || 1)}
                            placeholder="1"
                        />
                    </div>
                </div>

                {/* Papers Selection */}
                <div className="grid gap-3">
                    <div className="flex items-center justify-between">
                        <div>
                            <Label>Papers *</Label>
                            <p className="text-xs text-gray-500 mt-1">Only approved papers can be included in issues</p>
                        </div>
                        <span className="text-xs text-gray-500">
                            {selectedPapers.length} paper{selectedPapers.length !== 1 ? 's' : ''} selected
                        </span>
                    </div>
                    
                    {/* Display selected papers */}
                    <div className="space-y-2 max-h-48 overflow-y-auto">
                        {selectedPapers.map((paper) => (
                            <div key={paper.id} className="flex items-center justify-between p-3 border rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors">
                                <div className="flex flex-col flex-grow">
                                    <span className="font-medium text-sm">{paper.name}</span>
                                    <span className="text-xs text-gray-500">Status: {paper.status}</span>
                                    <span className="text-xs text-gray-400">Submitted: {new Date(paper.submissionDate).toLocaleDateString()}</span>
                                </div>
                                <Button
                                    type="button"
                                    variant="ghost"
                                    size="sm"
                                    onClick={() => handleRemovePaper(paper.id)}
                                    className="text-red-500 hover:text-red-700 hover:bg-red-50"
                                >
                                    <XIcon className="h-4 w-4" />
                                </Button>
                            </div>
                        ))}
                    </div>

                    {/* Add paper dropdown */}
                    {availableToAdd.length > 0 ? (
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Button variant="outline" className="w-full justify-between border-dashed border-2 hover:border-solid">
                                    <span className="text-gray-500">+ Add a paper</span>
                                    <ChevronDownIcon className="h-4 w-4" />
                                </Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent className="w-full min-w-[400px] max-h-60 overflow-y-auto">
                                {availableToAdd.map((paper) => (
                                    <DropdownMenuItem 
                                        key={paper.id}
                                        onClick={() => handleAddPaper(paper)}
                                        className="flex flex-col items-start cursor-pointer p-3"
                                    >
                                        <span className="font-medium">{paper.name}</span>
                                        <span className="text-xs text-gray-500">Status: {paper.status}</span>
                                        <span className="text-xs text-gray-400">Submitted: {new Date(paper.submissionDate).toLocaleDateString()}</span>
                                    </DropdownMenuItem>
                                ))}
                            </DropdownMenuContent>
                        </DropdownMenu>
                    ) : (
                        <div className="text-center py-4 text-gray-500 text-sm">
                            {selectedJournal ? 'No approved papers available for this journal' : 'Select a journal first to see available papers'}
                        </div>
                    )}
                </div>

                {/* Error display */}
                {error && (
                    <p className="text-red-600 text-sm">{error}</p>
                )}

                {/* Submit button */}
                <Button 
                    type="submit" 
                    className="w-full cursor-pointer" 
                    disabled={loading}
                >
                    {loading ? 'Creating Issue...' : 'Create Issue'}
                </Button>
            </div>
        </form>
    )
}
